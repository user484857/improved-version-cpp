"""
Combined Dashboard A — Standalone Flask Server (port 8060)
...
# Ensure local modules (analytics.py, demo_db_player.py, etc.) are importable
...
DASH_DIR = os.path.join(BASE_DIR, "dashboards", "combined-a")
...
def init_demo():
    """Start DemoDBPlayer from factory.db."""
    global data_source, mode
    if not os.path.exists(DB_PATH):
        print(f"  WARNING: {DB_PATH} not found — /api/data will return empty")
        return
...
@app.route("/dashboards/combined-a/<path:path>")
def dash_files(path):
...
# ── Live data (from DemoDBPlayer or OPC UA) ──────────────────
...
# ── Analytics API (always from factory.db) ───────────────────
...
def init_live():
    """Start LivePLCPlayer — reads PLC variables into RAM via OPC UA."""
    global data_source, mode
    from live_plc_player import LivePLCPlayer
    data_source = LivePLCPlayer(poll_interval=0.5)
    data_source.start()
...
if __name__ == "__main__":
    ...
    if "--live" in sys.argv:
        ...
        init_demo()
    print(f"\n  Combined Dashboard A — http://localhost:8060\n")
